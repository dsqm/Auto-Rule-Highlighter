...
### 样式配置
- 每个关键词可独立配置**背景色、文字颜色、字号（相对倍数）、粗体/斜体/下划线**，各项均带开关：关闭即不染色（背景关闭 = 无背景，文字关闭 = 保持页面原色）。
- 设置文字颜色时自动填充与背景匹配的反色值，可再手动调整。
- 样式预设库（设置页可增删改、拖拽排序）：第一项为**全局默认样式**；各处样式选择区均为「方形当前样式预览 + 圆形预设 + 自定义样式」布局，点击预设或自定义后预览方块实时同步。
